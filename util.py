...
def draw_ref_seq(SVG, x, y, ref, seq, start, cigar):
    svg = '<g transform=\"translate(' + str(SVG.hshift_size) + ',' + str(
        y + SVG.vshift_size) + ')\" font-size=\"' + str(SVG.font_size) + '\" text-anchor=\"middle\">'
    ref_idx = 0
    seq_idx = start
    while seq_idx < len(seq) and ref_idx < len(ref):
        svg += '<text x=\"' + str(ref_idx * SVG.block_size) + \
            '\" >' + str(seq[seq_idx]) + '</text>'
        ref_idx += 1
        seq_idx += 1
    svg += '</g>\n'
    return svg
...
def xmap_partial_svg(SVG, cmap, bnx, piles, max_subtracks, track_names, track):
    ...
    for pile in piles:
        ...
        for e in pile:
            ...
            trimmed_end = SVG.border['left'] + \
                min(end / SVG.zoom, SVG.dist / SVG.zoom)
            svg += '\n'
            x = trimmed_start
            w = trimmed_end - trimmed_start
            svg += add_svg_rect(SVG, x, y, w, h,
                                SVG.track_style(SVG.line_colour[track]))
            update_depth(SVG, y, h)
            svg += add_nicks(SVG, [cmap[int(nick)]
                                   for nick, label in e.Alignment], y, h)
            '''
...
def reference_location(SVG):
    svg = '\n' + '<g writing-mode=\"tb-rl\" fill=\"black\" font-size=\"' + \
        str(SVG.font_size) + '\">'
    nr = SVG.ref_loc_nr
    for i in range(nr):
        # positions
        if SVG.type == 'SAM':
            ...
            text = str(
                int((SVG.begin + i * (SVG.dist - 1) / (nr - 1)) / 1000)) + 'kbp'
        x = SVG.text_shift + \
            SVG.border['left'] + i * \
            ((SVG.dist - 2) / SVG.zoom - SVG.text_shift) / (nr - 1)
        y = SVG.border['toptext']
        svg += '\n' + '<text transform=\"translate(' + str(x) + ', ' + str(
            y) + ')rotate(270)\" style=' + SVG.text_style + '>' + text + '</text>'
    svg += '\n</g>'
    # reference string
    svg += '\n' + '<g writing-mode=\"tb-rl\" fill=\"black\" font-size=\"8\">'
    x = SVG.border['lefttext']
    ...
    svg += '\n' + '<text transform=\"translate(' + str(x) + ', ' + str(
        y) + ')rotate(270)\" style=' + SVG.text_style + '>Ref</text>'
    svg += '\n</g>'
    return svg
...
def reference_partial_svg_sam(SVG, ref, extra):
    ...
    svg += draw_ref_seq(SVG, 0, 0, ref_substr, ref_substr,
                        0, SVG.track_style(SVG.reference_colour))
    for contig in extra['contigs']:
        for idx in [0, 1]:
            if SVG.view_range[0] < contig[idx] and contig[idx] < SVG.view_range[1]:
                ...
                svg += add_svg_rect(SVG, x, 0, 2, SVG.block_size,
...
def start_partial_svg(SVG):
    """Add the header and opening tags for the partial svg."""
    if SVG.type == 'SAM':
        w = SVG.dist * SVG.block_size + \
            SVG.border['left'] + SVG.border['right']
    elif SVG.type == 'XMAP':
    ...
    svg += '<rect x=\"0\" y=\"0\" width=\"110%\" height=\"110%\" style=' + \
        SVG.background_style + '/>\n'
...
def end_partial_svg():
    """Add the closing tags for the partial svg."""
    svg = '</g>\n'
    svg += '</g>\n'
    svg += '</svg>'
    return svg
